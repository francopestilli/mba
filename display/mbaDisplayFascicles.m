function [figureHandle, lightHandle, sHandle] = mbaDisplayFascicles(fibers,p)
%
...
% 	  'p.colorMap'            - A matlab colormap (e.g., map=hsv)
...
tic
fprintf('[%s] Displaying %i fascicles... ', mfilename,numel(fibers));

% This is the number of edges eac surface has
surfaceCorners = p.nSurfaceFaces + 1;
figureHandle = [];
...
showme  = find(numNodes >= p.nNodesMin);

%t = cell(length(showme));n=t;b=t;
parfor i_fiber = 1:length(showme)
    % Make a variable fiber radius:
    fr = p.radius + (p.radius/8) .* randn(size(fibers{showme(i_fiber)}));
...
switch p.colorType
    case {'single','uniform'}
        if (length(p.faceAlpha) ==1)
           p.faceAlpha = p.faceAlpha*ones(size(Z));
        end
    ...
    case {'map'}
        for i_fiber = 1:length(showme)     
            ...
            % coordinates
            % Make a surface for the fiber.
            if size(p.color{showme(i_fiber)},2) == size(Z{i_fiber},1)
                sHandle(i_fiber) = surf(X{i_fiber},Y{i_fiber},Z{i_fiber}, ...
                    reshape(repmat(p.color{showme(i_fiber)}',size(Z{i_fiber},2),1),size(Z{i_fiber},1),size(Z{i_fiber},2),3), ...
                    'edgecolor', 'none',  'AmbientStrength',0.9,'FaceAlpha',.9,'CDataMapping','direct');
                
            elseif size(p.color{showme(i_fiber)},1) == size(Z{i_fiber},1)
                sHandle(i_fiber) = surf(X{i_fiber},Y{i_fiber},Z{i_fiber}, ...
                    reshape(repmat(p.color{showme(i_fiber)},size(Z{i_fiber},2),1),size(Z{i_fiber},1),size(Z{i_fiber},2),3), ...
                    'edgecolor', 'none',  'AmbientStrength',0.9,'FaceAlpha',.9,'CDataMapping','direct');
                ...
                sHandle(i_fiber) = surf(X{i_fiber},Y{i_fiber},Z{i_fiber}, ...
                    repmat(p.color{showme(i_fiber)},1,size(Z{i_fiber},2)), ...
                    'edgecolor', 'none',  'AmbientStrength',0.9,'FaceAlpha',.9,'CDataMapping','direct');
            end
        end
        colormap('bone')
